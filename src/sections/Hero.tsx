...
export const HeroSection = () => {
  return <div className='py-32 relative z-0 overflow-x-clip '>
    ...
    <h1 className='font-serif text-3xl md:text-5xl text-center mt-7 tracking-wide bg-gradient-to-r from-emerald-300 to-sky-400 text-transparent bg-clip-text'>Building Exceptional User Experiences</h1>
    </div>
    <p className='mt-4 text-center max-w-3xl sm:text-sm m-auto text-white/70 md:text-lg'>Hi, I'm Zaid, a passionate front-end developer with a love for crafting clean, responsive, and user-friendly web experiences. I specialize in turning ideas into reality using modern technologies like React, Framer Motion, GSAP, NextJS, Tailwind CSS, and JavaScript. Whether it's building intuitive interfaces or optimizing performance, I thrive on creating seamless digital solutions that leave a lasting impression. Let's build something amazing together!</p>
    <div className='flex flex-col md:flex-row justify-center items-center mt-8 gap-4'>
      <button className='inline-flex items-center gap-2 border border-white/15 px-5 h-11 rounded-xl z-0'>
        <a href='#projects'><span className='font-semibold'>Explore My Work</span></a>
        <ArrowDown className ="size-4" />
      </button>
      <button className='inline-flex items-center gap-2 border border-white bg-white text-gray-900 px-5 h-11 rounded-xl z-0'>
        <a href="https://www.instagram.com/amfastafboi" target="_blank"><span>👋</span>
        <span className='font-semibold'>Let's Connnect</span></a>
      </button>
    </div>
  </div>
  </div>;
